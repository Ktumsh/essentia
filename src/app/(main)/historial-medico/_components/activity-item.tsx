...
interface ActivityItemProps {
  item: MedicalHistoryActivityWithDetails;
...
const ActivityItem = ({
  ...
  return (
    ...
    >
      ...
      <div className="flex-1">
        <div className="flex flex-col-reverse gap-2 md:flex-row md:items-center">
          <p className="text-xs md:text-sm">
            ...
            {getActionSelfText(item.action)}{" "}
            <span
              className="cursor-pointer font-medium hover:underline"
              onClick={() =>
                onViewDocument && onViewDocument(item.medicalHistoryId)
              }
            >
      ...
      >
        {item.action === "deleted" && onRestoreDocument && (
        ...
        {onViewDocument && !item.medicalHistory.isDeleted && (
