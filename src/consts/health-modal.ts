import type { ModalData } from "@/types/common";

export const HEALTH_MODAL_DATA: ModalData[] = [
  {
    id: 1,
    modalTitle: "Más Allá de la Felicidad y la Satisfacción",
    modalImage: "/modal/salud-article-01.webp",
    modalBody: `
      <section>
        <div class="h-32 rounded-md mb-5 shadow-lg shadow-black/30 overflow-hidden">
          <img class="!mt-0 w-full h-full object-cover object-top" src="/modal/salud-article-01.webp" alt="Más Allá de la Felicidad y la Satisfacción" />
        </div>
        <h3>Descubre el Bienestar Multidimensional</h3>
        <p>
          El bienestar no se trata solo de ser feliz o sentirse satisfecho con la vida. Un estudio ha analizado 10 dimensiones del bienestar psicológico en 21 países europeos para ofrecer una visión más completa de lo que realmente significa estar bien.
        </p>
        <h3>Dimensiones Clave del Bienestar</h3>
        <ul>
          <li><strong>Competencia:</strong> Sentirse capaz y eficiente.</li>
          <li><strong>Estabilidad Emocional:</strong> Mantener un equilibrio emocional.</li>
          <li><strong>Compromiso:</strong> Estar activamente involucrado en la vida.</li>
          <li><strong>Significado:</strong> Sentir que la vida tiene un propósito.</li>
          <li><strong>Optimismo:</strong> Tener una perspectiva positiva del futuro.</li>
          <li><strong>Emoción Positiva:</strong> Experimentar emociones agradables.</li>
          <li><strong>Relaciones Positivas:</strong> Tener conexiones saludables y de apoyo.</li>
          <li><strong>Resiliencia:</strong> Poder recuperarse de las adversidades.</li>
          <li><strong>Autoestima:</strong> Tener una buena valoración de uno mismo.</li>
          <li><strong>Vitalidad:</strong> Sentirse lleno de energía y vigor.</li>
        </ul>
        <p>
          Este enfoque multidimensional permite identificar áreas específicas donde se pueden hacer intervenciones para mejorar el bienestar general. Las relaciones positivas y la resiliencia se destacan como factores cruciales.
        </p>
        <p class="text-sm">
          Fuente del artículo: <a class="text-bittersweet-400 dark:text-cerise-red-600 font-medium" target="_blank" href="https://hqlo.biomedcentral.com/articles/10.1186/s12955-020-01423-y">BMC</a>
        </p>
      </section>
    `,
  },
  {
    id: 2,
    modalTitle: "Las 8 Dimensiones del Bienestar",
    modalImage: "/modal/salud-article-02.webp",
    modalBody: `
      <section>
        <div class="h-32 rounded-md mb-5 shadow-lg shadow-black/30 overflow-hidden">
          <img class="!mt-0 w-full h-full object-cover object-top" src="/modal/salud-article-02.webp" alt="Las 8 Dimensiones del Bienestar" />
        </div>
        <p>
          El bienestar integral abarca diferentes áreas de nuestra vida. Aquí están las ocho dimensiones clave y algunas actividades para mejorar en cada una de ellas:
        </p>
        <ul>
          <li><strong>Emocional:</strong> Gestiona el estrés con meditación y terapia.</li>
          <li><strong>Ambiental:</strong> Mantén tu espacio limpio y organizado.</li>
          <li><strong>Intelectual:</strong> Aprende continuamente nuevas habilidades.</li>
          <li><strong>Físico:</strong> Haz ejercicio regularmente y come saludablemente.</li>
          <li><strong>Social:</strong> Cultiva relaciones positivas y equilibradas.</li>
          <li><strong>Espiritual:</strong> Encuentra propósito y paz interior.</li>
          <li><strong>Ocupacional:</strong> Busca satisfacción y crecimiento en tu trabajo.</li>
          <li><strong>Financiero:</strong> Administra tus finanzas para una mayor seguridad.</li>
        </ul>
        <h3>Actividades Prácticas:</h3>
        <ul>
          <li><strong>Diario de Gratitud:</strong> Anota cosas positivas diariamente.</li>
          <li><strong>Organización:</strong> Mejora la iluminación y el orden de tus espacios.</li>
          <li><strong>Lectura Diaria:</strong> Lee sobre temas de interés y desarrollo.</li>
          <li><strong>Ejercicio:</strong> Participa en clases de yoga o gimnasio.</li>
          <li><strong>Socialización:</strong> Pasa tiempo con amigos y familiares.</li>
          <li><strong>Meditación:</strong> Practica mindfulness diariamente.</li>
          <li><strong>Capacitación:</strong> Asiste a cursos y talleres.</li>
          <li><strong>Planificación Financiera:</strong> Consulta con expertos y planifica tus gastos.</li>
        </ul>
        <p>
          Implementar estas prácticas puede ayudarte a alcanzar un bienestar integral. Para más detalles, visita el artículo completo.
        </p>
        <p class="text-sm">
          Fuente del artículo: <a class="text-bittersweet-400 dark:text-cerise-red-600 font-medium" target="_blank" href="https://learningforward.org/journal/looking-ahead/8-dimensions-of-wellness-for-educators/">Learning Forward</a>
        </p>
      </section>
    `,
  },
  {
    id: 3,
    modalTitle: "La Atención Plena para Tu Salud",
    modalImage: "/modal/salud-article-03.webp",
    modalBody: `
      <section>
        <div class="h-32 rounded-md mb-5 shadow-lg shadow-black/30 overflow-hidden">
          <img class="!mt-0 w-full h-full object-cover object-[50%_20%]" src="/modal/salud-article-03.webp" alt="La Atención Plena para Tu Salud" />
        </div>
        <p>
          La atención plena, o mindfulness, se trata de estar presente en el momento actual y observar nuestros pensamientos y sentimientos sin juicio. Originada en la meditación budista, se ha incorporado en escuelas, trabajos y hospitales debido a sus beneficios para la salud.
        </p>
        <h3>Beneficios de la Atención Plena</h3>
        <ul>
          <li><strong>Reducción del Estrés y Ansiedad:</strong> Ayuda a disminuir la ansiedad y la depresión.</li>
          <li><strong>Mejora del Sueño y la Presión Arterial:</strong> Contribuye a dormir mejor y bajar la presión arterial.</li>
          <li><strong>Manejo del Dolor:</strong> Ayuda a enfrentar el dolor crónico.</li>
          <li><strong>Hábitos Saludables:</strong> Fomenta elecciones saludables en la dieta y el ejercicio.</li>
        </ul>
        <h3>Prácticas de Mindfulness</h3>
        <ul>
          <li><strong>Respiración Consciente:</strong> Respira profundamente y enfócate en tu respiración.</li>
          <li><strong>Paseos Conscientes:</strong> Observa el entorno mientras caminas.</li>
          <li><strong>Comida Consciente:</strong> Saborea cada bocado, prestando atención a los sabores y texturas.</li>
          <li><strong>Escaneo Corporal:</strong> Observa las sensaciones en tu cuerpo, de los pies a la cabeza.</li>
        </ul>
        <p>
          Busca programas y aplicaciones de mindfulness en universidades y escuelas de medicina para asegurarte de que sean confiables y basados en evidencia.
        </p>
        <p class="text-sm">
          Fuente del artículo: <a class="text-bittersweet-400 dark:text-cerise-red-600 font-medium" target="_blank" href="https://newsinhealth.nih.gov/2021/06/mindfulness-your-health">NIH News in Health</a>
        </p>
      </section>
    `,
  },
  {
    id: 4,
    modalTitle: "Cómo Lograr un Estilo de Vida Saludable",
    modalImage: "/modal/salud-article-04.webp",
    modalBody: `
      <section>
        <div class="h-32 rounded-md mb-5 shadow-lg shadow-black/30 overflow-hidden">
          <img class="!mt-0 w-full h-full object-cover object-bottom" src="/modal/salud-article-04.webp" alt="Cómo Lograr un Estilo de Vida Saludable" />
        </div>
        <p>
          Seguir un estilo de vida adecuado implica vivir en armonía y cuidar de la salud para tener un mejor bienestar. Aquí hay seis factores clave para lograrlo:
        </p>
        <ul>
          <li><strong>Alimentación Equilibrada</strong>
            <div>
              Consume una variedad de alimentos como frutas, verduras, proteínas y grasas saludables. Esto te proporcionará los nutrientes necesarios para mantenerte enérgico y saludable.
            </div>
          </li>
          <li><strong>Actividad Física Regular</strong>
            <div>
              Realiza ejercicios moderados al menos 150 minutos por semana. Esto puede incluir caminar, correr o bailar, lo cual ayuda a mantener tu cuerpo en forma y tu mente activa.
            </div>
          </li>
          <li><strong>Dormir Bien</strong>
            <div>
              Dormir adecuadamente mejora la memoria, el sistema inmunitario y reduce el estrés. Asegúrate de tener un sueño reparador para recargar energías.
            </div>
          </li>
          <li><strong>Control del Estrés</strong>
            <div>
              Identifica y maneja las causas del estrés mediante técnicas de relajación, como la meditación, y mantén una actitud positiva. El estrés prolongado puede afectar tu salud física y mental.
            </div>
          </li>
          <li><strong>Actividades Sociales</strong>
            <div>
              Participa en actividades sociales y mantén contacto con amigos y familiares. Las relaciones sociales saludables son cruciales para el bienestar emocional.
            </div>
          </li>
          <li><strong>Mantener un Peso Saludable</strong>
            <div>
              Combina una dieta balanceada con ejercicio regular y un buen descanso. Mantener un peso adecuado es importante para la salud general y la prevención de enfermedades.
            </div>
          </li>
        </ul>
        <p class="text-sm">
          Fuente del artículo: <a class="text-bittersweet-400 dark:text-cerise-red-600 font-medium" target="_blank" href="https://www.olivetto.com.co/bienestar/estilo-de-vida-adecuado-asi-se-logra/">Olivetto</a>.
        </p>
      </section>
    `,
  },
  {
    id: 5,
    modalTitle: "8 Consejos para Mejorar tu Bienestar Físico y Emocional",
    modalImage: "/modal/salud-article-05.webp",
    modalBody: `
      <section>
        <div class="h-32 rounded-md mb-5 shadow-lg shadow-black/30 overflow-hidden">
          <img class="!mt-0 w-full h-full object-cover object-center" src="/modal/salud-article-05.webp" alt="8 Consejos para Mejorar tu Bienestar Físico y Emocional" />
        </div>
        <p>
          Seguir un estilo de vida adecuado implica vivir en armonía y cuidar de la salud para tener un mejor bienestar. Aquí hay ocho consejos clave para lograrlo:
        </p>
        <ul>
          <li><strong>Come Sano e Inteligente</strong>
            <div>
              Incluye una variedad de alimentos nutritivos y evita el exceso de sal y azúcar. Opta por frutas, verduras, proteínas magras y granos enteros para una dieta equilibrada.
            </div>
          </li>
          <li><strong>Evita el Alcohol</strong>
            <div>
              Reduce su consumo para proteger tu salud física y mental. El consumo excesivo de alcohol puede llevar a diversos problemas de salud.
            </div>
          </li>
          <li><strong>Duerme Bien</strong>
            <div>
              Asegúrate de dormir 7-8 horas diarias. Un buen descanso es crucial para el funcionamiento óptimo del cuerpo y la mente.
            </div>
          </li>
          <li><strong>Deja de Fumar</strong>
            <div>
              El tabaco perjudica gravemente la salud. Dejar de fumar mejora la salud cardiovascular y reduce el riesgo de enfermedades graves.
            </div>
          </li>
          <li><strong>Mantén Buena Higiene</strong>
            <div>
              Mantén tu entorno limpio y sigue una rutina de higiene personal. Esto incluye lavarse las manos regularmente y mantener una buena higiene dental.
            </div>
          </li>
          <li><strong>Haz lo que te Hace Feliz</strong>
            <div>
              Realiza actividades que disfrutes y te hagan feliz. Participar en hobbies y actividades recreativas puede mejorar tu estado de ánimo y bienestar general.
            </div>
          </li>
          <li><strong>Sonríe</strong>
            <div>
              Las emociones positivas mejoran el bienestar mental. Sonreír y mantener una actitud positiva puede reducir el estrés y aumentar la felicidad.
            </div>
          </li>
          <li><strong>Toma el Sol</strong>
            <div>
              Disfruta de la luz solar para obtener vitamina D, que es esencial para la salud ósea y el sistema inmunológico.
            </div>
          </li>
        </ul>
        <p class="text-sm">
          Fuente del artículo: <a class="text-bittersweet-400 dark:text-cerise-red-600 font-medium" target="_blank" href="https://www.naturelorganic.cl/consejos-para-mejorar-bienestar-fisico-emocional">Naturel Organic</a>.
        </p>
      </section>
    `,
  },
  {
    id: 6,
    modalTitle: "Consejos de Bienestar Diario",
    modalImage: "/modal/salud-article-06.webp",
    modalBody: `
      <section>
        <div class="h-32 rounded-md mb-5 shadow-lg shadow-black/30 overflow-hidden">
          <img class="!mt-0 w-full h-full object-cover object-[50%_20%]" src="/modal/salud-article-06.webp" alt="Consejos de Bienestar Diario" />
        </div>
        <p>
          Aquí hay algunos consejos clave para mejorar tu bienestar físico y emocional:
        </p>
        <ul>
          <li><strong>Reconocer Sentimientos:</strong> Identifica cómo te sientes y respira profundamente para calmarte.
          </li>
          <li><strong>Búsqueda del Tesoro:</strong> Hazlo mientras caminas para reflexionar y despejar la mente.
          </li>
          <li><strong>Habla:</strong> Comparte tus problemas con alguien de confianza para aliviarlos.
          </li>
          <li><strong>Duerme Bien:</strong> Desconéctate de las pantallas antes de dormir y escribe tus pensamientos.
          </li>
          <li><strong>Alimentación Saludable:</strong> Opta por una dieta balanceada con frutas y verduras.
          </li>
          <li><strong>Prácticas Meditativas:</strong> La meditación y la oración pueden reducir el estrés.
          </li>
          <li><strong>Ejercicio Regular:</strong> El ejercicio mejora tanto el cuerpo como la mente.
          </li>
          <li><strong>Conexiones Sociales:</strong> Mantén relaciones cercanas con amigos y familiares.
          </li>
          <li><strong>Higiene y Salud Personal:</strong> Cuida tu higiene personal y mantén tu entorno limpio.
          </li>
          <li><strong>Autocuidado:</strong> Realiza actividades que disfrutes y te hagan feliz.
          </li>
          <li><strong>Toma de Sol:</strong> Disfruta de la luz solar para obtener vitamina D.
          </li>
          <li><strong>Diversión y Recreación:</strong> Incluye tiempo para hobbies y actividades recreativas.
          </li>
          <li><strong>Rutina y Organización:</strong> Mantén una rutina diaria y organiza tus tareas.
          </li>
          <li><strong>Actitud Positiva:</strong> Mantén una actitud positiva para mejorar tu bienestar emocional.
          </li>
          <li><strong>Desarrollo Personal:</strong> Busca oportunidades de aprendizaje y crecimiento personal.
          </li>
          <li><strong>Hidratación:</strong> Bebe suficiente agua durante el día.
          </li>
          <li><strong>Consumo Responsable:</strong> Evita el consumo excesivo de alcohol y deja de fumar.
          </li>
        </ul>
        <p class="text-sm">
          Fuente del artículo: <a class="text-bittersweet-400 dark:text-cerise-red-600 font-medium" target="_blank" href="https://www.saludsiemprevc.org/consejos-de-bienestar">Salud Siempre</a>.
        </p>
      </section>
    `,
  },
];
